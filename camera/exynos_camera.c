 ...
 * Modified for P4NOTELTE: Scott Brissenden
 *
...
int exynos_camera_capture_start(struct exynos_camera *exynos_camera, int is_capture)
{
	...
	if(!is_capture || exynos_camera->preview_output_enabled || exynos_camera->camera_fimc_is) {
	...
	}else if(!exynos_camera->camera_fimc_is){
...
/*	rc = exynos_v4l2_s_ctrl(exynos_camera, 0, V4L2_CID_CACHEABLE, 1);
	...
	if(exynos_camera->camera_fimc_is){
		for (i = EXYNOS_FRONT_CAMERA_CAPTURE_BUFFERS_COUNT; i > 0; i--) {
			rc = exynos_v4l2_reqbufs_cap(exynos_camera, 0, i);
			if (rc >= 0)
				break;
		}
	}else if(!is_capture || exynos_camera->preview_output_enabled) {
	...
	if(!is_capture && !exynos_camera->camera_fimc_is){
...
int exynos_camera_preview_output_start(struct exynos_camera *exynos_camera)
{
	...
	if(exynos_camera->camera_fimc_is)
		output->buffers_count = EXYNOS_FRONT_CAMERA_PREVIEW_BUFFERS_COUNT;
	else
